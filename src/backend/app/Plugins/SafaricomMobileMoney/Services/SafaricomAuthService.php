...
class SafaricomAuthService {
    ...
    private function generateAccessToken(SafaricomCredential $credential): string {
        ...
        if (!$response->successful()) {
            // Daraja returns 400/401 for invalid credentials and an HTML 500
            // when the OAuth gateway itself is having a bad day. Either way
            // we don't want the HTML body leaking into the operator-facing
            // error toast — log the raw body and surface a clean reason.
