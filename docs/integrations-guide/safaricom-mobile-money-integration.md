...
> [!INFO]
> The **operator** initiates STK Push from the MPM sidebar (**Safaricom M-PESA → Initiate Payment**). There is no customer-facing public payment URL for M-PESA — that flow only makes sense for hosted redirect providers.
...
- **Daraja code `4999`:**
  - Undocumented sandbox transient. The plugin recognises it and keeps polling — you should not see this surface as a "Failed" result. If you do, the dev server may be serving a stale frontend bundle; hard-refresh.
